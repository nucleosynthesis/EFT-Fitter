...
class fitter:
  ...
  def setNuisances(self,key_vals):
    ...
    #for k,v in key_vals.items():
      #ind = self.npindex[k]
    #[self.nps[k]=v for k,v in key_vals.items()] #  self.nps[k] = v
  ...
  def minimize(self,freezePOIS=[],verbose=True):
    ...
    #self.FitResult = minimize(GetChi2,PToFit,args=self,bounds=PToFitBounds,options={'ftol':1e-3,'eps':1e-3})
    ...
    #for ip, ipoi in enumerate(self.PToFitList): 
  ...
  def scan_fixed(self,poi,npoints=1000,reset=True):
    ...
    for p in pvals:
      ...
      #print("at , %s=%g"%(poi,p)," chi2=%g"%(self.getChi2(verbose=False)-nll_global))
      #for _input in self.INPUTS:
      #  for i,x in enumerate(_input.XList):
      #    print(x,self.evaluateScalingFunctions(_input.XList[i]),end='  ')
      #print("... >")
